...
void get_child_stats(int status, pid_t pid);
pid_t execute_external_cmd(char **, int);
void remove_non_alphanum(char *);
...
void waitchild(char buffer[BUFLEN])
{
	...
	char *endptr;
	pid_t pid = (pid_t) strtol(number, &endptr, 10);
	...
	if((*number != '\0')&&(*endptr == '\0'))
	{
		...
		if(strcmp(number, "-1")==0x0){
		    while(childnum > 0){
                pid = wait(&status);
                get_child_stats(status, pid);
                childnum--;
                i++;
		    }
...
void get_child_stats(int status, pid_t pid){
    if(WIFEXITED(status)){
        ...
        printf("-xssh child exited with return code %d\n", WEXITSTATUS(status));
    }

    else if(WIFSIGNALED(status)){
        printf("-xssh: process %s caught signal\n", pid);
...
void remove_non_alphanum(char *val){

    for(int i = 0; i < strlen(val); i++){
        if(isalnum(val[i]) == 0x0){
            val[i] = ' ';
        }
    }
}

pid_t execute_external_cmd(char **argv, int backflag){
    ...
    switch(pid = fork()){
        case 0:
            retcode = execvp(*argv, argv);
            fprintf(stderr, "-xssh: Unable to execute the instruction %s\n", argv[0]);
            exit(-1);
        case -1:
            perror("fork");
            return -2;
...
int program(char buffer[BUFLEN])
{
    ...
    parse(buffer, argv);
    pid = execute_external_cmd(argv, backflag);
